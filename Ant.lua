...
-- Input untuk Fly Height
local flyBox = createInput("Fly Y:", 90, "50")
...
-- Tombol Fly
createToggleButton("FlyToggle", 130, "✈️ Fly: ON", "✈️ Fly: OFF", function(state)
	flyEnabled = state
	...
	if flyEnabled then
		...
		if rootPart then
			-- Nonaktifkan physics biasa
			humanoid.PlatformStand = true

			-- Buat BodyVelocity
			flyVelocity = Instance.new("BodyVelocity")
			flyVelocity.Velocity = Vector3.new(0, flyHeight, 0)
			...
			platformConnection = RunService.RenderStepped:Connect(function()
				if flyVelocity and rootPart then
					flyVelocity.Velocity = Vector3.new(0, flyHeight, 0)
				end
			end)
		end
